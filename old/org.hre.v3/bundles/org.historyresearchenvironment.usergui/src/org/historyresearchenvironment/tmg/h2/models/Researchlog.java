package org.historyresearchenvironment.tmg.h2.models;

import java.math.BigDecimal;

/**
 * The persistent class for the RESEARCHLOG database table.
 *
 * INSERT INTO RESEARCHLOG (RLTYPE, RLNUM, RLPER1, RLPER2, RLGTYPE, TASK,
 * RLEDITED, DESIGNED, BEGUN, PROGRESS, COMPLETED, PLANNED, EXPENSES, COMMENTS,
 * RLNOTE, KEYWORDS, DSID, ID_PERSON, ID_EVENT, ID_SOURCE, ID_REPOS, TT,
 * REFERENCE, RECNO ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
 * )
 */
public class Researchlog {

	private String begun;

	private String comments;

	private String completed;

	private String designed;

	private int dsid;

	private BigDecimal expenses;

	private int idEvent;

	private int idPerson;

	private int idRepos;

	private int idSource;

	private String keywords;

	private BigDecimal numRec;

	private String planned;

	private String progress;

	private int recno;

	private String reference;

	private String rledited;

	private int rlgtype;

	private String rlnote;

	private int rlnum;

	private int rlper1;

	private int rlper2;

	private String rltype;

	private String s;

	private String task;

	private String tt;

	public Researchlog() {
	}

	public String getBegun() {
		return this.begun;
	}

	public String getComments() {
		return this.comments;
	}

	public String getCompleted() {
		return this.completed;
	}

	public String getDesigned() {
		return this.designed;
	}

	public int getDsid() {
		return this.dsid;
	}

	public BigDecimal getExpenses() {
		return this.expenses;
	}

	public int getIdEvent() {
		return this.idEvent;
	}

	public int getIdPerson() {
		return this.idPerson;
	}

	public int getIdRepos() {
		return this.idRepos;
	}

	public int getIdSource() {
		return this.idSource;
	}

	public String getKeywords() {
		return this.keywords;
	}

	public BigDecimal getNumRec() {
		return this.numRec;
	}

	public String getPlanned() {
		return this.planned;
	}

	public String getProgress() {
		return this.progress;
	}

	public int getRecno() {
		return this.recno;
	}

	public String getReference() {
		return this.reference;
	}

	public String getRledited() {
		return this.rledited;
	}

	public int getRlgtype() {
		return this.rlgtype;
	}

	public String getRlnote() {
		return this.rlnote;
	}

	public int getRlnum() {
		return this.rlnum;
	}

	public int getRlper1() {
		return this.rlper1;
	}

	public int getRlper2() {
		return this.rlper2;
	}

	public String getRltype() {
		return this.rltype;
	}

	public String getS() {
		return this.s;
	}

	public String getTask() {
		return this.task;
	}

	public String getTt() {
		return this.tt;
	}

	public void setBegun(String begun) {
		this.begun = begun;
	}

	public void setComments(String comments) {
		this.comments = comments;
	}

	public void setCompleted(String completed) {
		this.completed = completed;
	}

	public void setDesigned(String designed) {
		this.designed = designed;
	}

	public void setDsid(int dsid) {
		this.dsid = dsid;
	}

	public void setExpenses(BigDecimal expenses) {
		this.expenses = expenses;
	}

	public void setIdEvent(int idEvent) {
		this.idEvent = idEvent;
	}

	public void setIdPerson(int idPerson) {
		this.idPerson = idPerson;
	}

	public void setIdRepos(int idRepos) {
		this.idRepos = idRepos;
	}

	public void setIdSource(int idSource) {
		this.idSource = idSource;
	}

	public void setKeywords(String keywords) {
		this.keywords = keywords;
	}

	public void setNumRec(BigDecimal numRec) {
		this.numRec = numRec;
	}

	public void setPlanned(String planned) {
		this.planned = planned;
	}

	public void setProgress(String progress) {
		this.progress = progress;
	}

	public void setRecno(int recno) {
		this.recno = recno;
	}

	public void setReference(String reference) {
		this.reference = reference;
	}

	public void setRledited(String rledited) {
		this.rledited = rledited;
	}

	public void setRlgtype(int rlgtype) {
		this.rlgtype = rlgtype;
	}

	public void setRlnote(String rlnote) {
		this.rlnote = rlnote;
	}

	public void setRlnum(int rlnum) {
		this.rlnum = rlnum;
	}

	public void setRlper1(int rlper1) {
		this.rlper1 = rlper1;
	}

	public void setRlper2(int rlper2) {
		this.rlper2 = rlper2;
	}

	public void setRltype(String rltype) {
		this.rltype = rltype;
	}

	public void setS(String s) {
		this.s = s;
	}

	public void setTask(String task) {
		this.task = task;
	}

	public void setTt(String tt) {
		this.tt = tt;
	}

}