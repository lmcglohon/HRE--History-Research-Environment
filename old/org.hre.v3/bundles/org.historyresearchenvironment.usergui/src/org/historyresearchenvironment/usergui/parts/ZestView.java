...
public class ZestView extends AbstractHREGuiPart implements IZoomableWorkbenchPart {
	...
	public void postConstruct(Composite parent) {
		...
		final ZestPersonNodeModelContentProvider model = new ZestPersonNodeModelContentProvider(1);
	...
	private void subscribePersonUpdateTopic(@UIEventTopic(HreEventConstants.PERSON_UPDATE_TOPIC) int perNo) {
		final ZestPersonNodeModelContentProvider model = new ZestPersonNodeModelContentProvider(perNo);
		viewer.setInput(model.getNodes());
		viewer.refresh();
	}
