...
public abstract class LogFactory {
	 ...
	 * specify the priority of that particular config file. The associated value is
	 * a floating-point number; higher values take priority over lower values.
	 */
	public static final String PRIORITY_KEY = "priority";

	/**
	 * The name (<code>use_tccl</code>) of the key in the config file used to
	 * specify whether logging classes should be loaded via the thread context class
	 * loader (TCCL), or not. By default, the TCCL is used.
	 */
	public static final String TCCL_KEY = "use_tccl";

	/**
	 * The name (<code>org.apache.commons.logging.LogFactory</code>) of the property
	 * used to identify the LogFactory implementation class name. This can be used
	 * as a system property, or as an entry in a configuration properties file.
	 */
	...
	/**
	 * The name (<code>commons-logging.properties</code>) of the properties file to
	 * search for.
	 */
	...
	/**
	 * The name (<code>org.apache.commons.logging.diagnostics.dest</code>) of the
	 * property used to enable internal commons-logging diagnostic output, in order
	 * to get information on what logging implementations are being discovered, what
	 * classloaders they are loaded through, etc.
	 * <p>
	 * If a system property of this name is set then the value is assumed to be the
	 * name of a file. The special strings STDOUT or STDERR (case-sensitive)
	 * indicate output to System.out and System.err respectively.
	 * <p>
	 * Diagnostic logging should be used only to debug problematic configurations
	 * and should not be set in normal production use.
	 */
	 ...
	 * method, so that users can clearly see which LogFactory class is generating
	 * the output.
	 */
	 ...
	 * <code>org.apache.commons.logging.LogFactory.HashtableImpl</code>) value
	 * allows the <code>Hashtable</code> used to store classloaders to be
	 * substituted by an alternative implementation.
	 * <p>
	 ...
	 * <p>
	 * <strong>Usage:</strong> Set this property when Java is invoked and
	 * <code>LogFactory</code> will attempt to load a new instance of the given
	 * implementation class. For example, running the following ant scriplet:
	 * 
	 * <pre>
	 ...
	 * <p>
	 * A typical use case is to allow a custom Hashtable implementation using weak
	 * references to be substituted. This will allow classloaders to be garbage
	 * collected without the need to release them (on 1.3+ JVMs only, of course ;).
	 */
	...
	/**
	 * A reference to the classloader that loaded this class. This is the same as
	 * LogFactory.class.getClassLoader(). However computing this value isn't quite
	 * as simple as that, as we potentially need to use AccessControllers etc. It's
	 * more efficient to compute it once and cache it here.
	 */
	...
	/**
	 * The previously constructed <code>LogFactory</code> instances, keyed by the
	 * <code>ClassLoader</code> with which it was created.
	 */
	protected static Hashtable<ClassLoader, LogFactory> factories = null;
	 ...
	 * <ul>
	 * <li>using JDK1.1 and the calling code is loaded via the system classloader
	 * (very common)</li>
	 * <li>using JDK1.2+ and the calling code is loaded via the boot classloader
	 * (only likely for embedded systems work).</li>
	 * </ul>
	 * Note that <code>factories</code> is a <i>Hashtable</i> (not a HashMap), and
	 * hashtables don't allow null as a key.
	 * 
	...
	/**
	 * Remember this factory, so later calls to LogFactory.getCachedFactory can
	 * return the previously created object (together with all its cached Log
	 * objects).
	 *
	 * @param classLoader
	 *            should be the current context classloader. Note that this can be
	 *            null under some circumstances; this is ok.
	 * @param factory
	 ...
	 * @param factoryClass
	 * @param classLoader
	 *            used to load the specified factory class. This is expected to be
	 *            either the TCCL or the classloader which loaded this class. Note
	 *            that the classloader which loaded this class might be "null" (ie
	 *            the bootloader) for embedded systems.
	...
	protected static Object createFactory(String factoryClass, ClassLoader classLoader) {
		...
		try {
			if (classLoader != null) {
				try {
					...
					if (LogFactory.class.isAssignableFrom(logFactoryClass)) {
						if (isDiagnosticsEnabled()) {
							logDiagnostic("Loaded class " + logFactoryClass.getName() + " from classloader "
						...
						//
						if (isDiagnosticsEnabled()) {
							logDiagnostic("Factory class " + logFactoryClass.getName() + " loaded from classloader "
									+ objectId(logFactoryClass.getClassLoader()) + " does not extend '"
									+ LogFactory.class.getName() + "' as loaded by this classloader.");
					...
					if (classLoader == thisClassLoader) {
						...
						if (isDiagnosticsEnabled()) {
							logDiagnostic("Unable to locate any class called '" + factoryClass + "' via classloader "
									+ objectId(classLoader));
						}
						throw ex;
					}
					...
					if (classLoader == thisClassLoader) {
						// Nothing more to try, onwards.
						if (isDiagnosticsEnabled()) {
							logDiagnostic("Class '" + factoryClass + "' cannot be loaded" + " via classloader "
									+ objectId(classLoader)
					...
					if (classLoader == thisClassLoader) {
						// There's no point in falling through to the code below
						// that
						// tries again with thisClassLoader, because we've just
						// tried
						...
						//
						// Construct a good message: users may not actual expect
						// that a custom implementation
						// has been specified. Several well known containers use
						// this mechanism to adapt JCL
					...
					// class is in
					// a shared classpath but the TCCL has a copy of LogFactory
					// and the
					// specified LogFactory implementation; we will fall back to
					// using the
			...
			/*
			 * At this point, either classLoader == null, OR classLoader was unable to load
			 * factoryClass.
			 * 
			 * In either case, we call Class.forName, which is equivalent to
			 * LogFactory.class.getClassLoader().load(name), ie we ignore the classloader
			 * parameter the caller passed, and fall back to trying the classloader
			 * associated with this class. See the javadoc for the newFactory method for
			...
			if ((logFactoryClass != null) && !LogFactory.class.isAssignableFrom(logFactoryClass)) {
				return new LogConfigurationException("The chosen LogFactory implementation does not extend LogFactory."
	 ...
	 * (context-classloader -> logfactory-object). Version 1.2+ of Java supports
	 * "weak references", allowing a custom Hashtable class to be used which uses
	 * only weak references to its keys. Using weak references can fix memory leaks
	 * on webapp unload in some cases (though not all). Version 1.1 of Java does not
	 * support weak references, so we must dynamically determine which we are using.
	 * And just for fun, this code also supports the ability for a system property
	 * to specify an arbitrary Hashtable implementation name.
	 * <p>
	 * Note that the correct way to ensure no memory leaks occur is to ensure that
	 * LogFactory.release(contextClassLoader) is called whenever a webapp is
	 * undeployed.
	 */
	...
	private static final Hashtable<ClassLoader, LogFactory> createFactoryStore() {
		...
		try {
			...
			result = (Hashtable<ClassLoader, LogFactory>) implementationClass.newInstance();
		} catch (final Throwable t) {
			handleThrowable(t); // may re-throw t

			// ignore
			if (!WEAK_HASHTABLE_CLASSNAME.equals(storeImplementationClass)) {
				// if the user's trying to set up a custom implementation, give
				// a clue
				if (isDiagnosticsEnabled()) {
		...
		if (result == null) {
			result = new Hashtable<ClassLoader, LogFactory>();
		}
		return result;
	}
	 ...
	 * <p>
	 * Most/all code should call getContextClassLoaderInternal rather than calling
	 * this method directly.
	 * <p>
	 * The thread context class loader is available for JDK 1.2 or later, if certain
	 * security conditions are met.
	 * <p>
	 * Note that no internal logging is done within this method because this method
	 * is called every time LogFactory.getLogger() is called, and we don't want too
	 * much output generated here.
	 *
	 ...
	 *         classes in the current call stack.
	 * @since 1.1
	 */
	protected static ClassLoader directGetContextClassLoader() throws LogConfigurationException {
		...
		try {
			...
			/**
			 * getContextClassLoader() throws SecurityException when the context class
			 * loader isn't an ancestor of the calling class's class loader, or if security
			 * permissions are restricted.
			 *
			 * We ignore this exception to be consistent with the previous behavior (e.g.
			 * 1.1.3 and earlier).
			 */
			// ignore
		}
	 ...
	 *            is the context classloader associated with the current thread.
	 *            This allows separate LogFactory objects per component within a
	 *            container, provided each component has a distinct context
	 *            classloader set. This parameter may be null in JDK1.1, and in
	 *            embedded systems where jcl-using code is placed in the
	 *            bootclasspath.
	 *
	 * @return the factory associated with the specified classloader if one has
	 *         previously been created, or null if this is the first time we have
	 ...
	 * <p>
	 * Theoretically, calling getClassLoader can throw a security exception, and so
	 * should be done under an AccessController in order to provide maximum
	 * flexibility. However in practice people don't appear to use security policies
	 * that forbid getClassLoader calls. So for the moment all code is written to
	 * call this method rather than Class.getClassLoader, so that we could put
	 * AccessController stuff in this method without any disruption later if we need
	 * to.
	 * <p>
	 * Even when using an AccessController, however, this method can still throw
	 * SecurityException. Commons-logging basically relies on the ability to access
	 * classloaders, ie a policy that forbids all classloader access will also
	 * prevent commons-logging from working: currently this method will throw an
	 * exception preventing the entire app from starting up. Maybe it would be good
	 * to detect this situation and just disable all commons-logging? Not high
	 * priority though - as stated above, security policies that prevent classloader
	 * access aren't common.
	 * <p>
	 * Note that returning an object fetched via an AccessController would
	 * technically be a security flaw anyway; untrusted code that has access to a
	 * trusted JCL library could use it to fetch the classloader for a class even
	 * when forbidden to do so directly.
	 *
	 * @since 1.1
	 */
	protected static ClassLoader getClassLoader(Class<LogFactory> clazz) {
		try {
			...
			if (isDiagnosticsEnabled()) {
				logDiagnostic("Unable to get classloader for class '" + clazz + "' due to security restrictions - "
						+ ex.getMessage());
			}
			throw ex;
		}
	 ...
	 * value for its PRIORITY property is returned. If multiple files have the same
	 * PRIORITY value then the first in the classpath is returned.
	 * <p>
	 ...
	 * override logging settings in all webapps by placing a
	 ...
	 * priorities which are in the webapps. Webapps can also use explicit priorities
	 * to override a configuration file in the shared classpath if needed.
	 */
				...
				if (newProps != null) {
					if (props == null) {
						...
						if (newPriority > priority) {
							if (isDiagnosticsEnabled()) {
								logDiagnostic("[LOOKUP] Properties file at '" + url + "'" + " with priority "
										+ newPriority + " overrides file at '" + propsUrl + "'" + " with priority "
										+ priority);
							}
							...
							if (isDiagnosticsEnabled()) {
								logDiagnostic("[LOOKUP] Properties file at '" + url + "'" + " with priority "
										+ newPriority + " does not override file at '" + propsUrl + "'"
	 ...
	 * <p>
	 * In versions prior to 1.1, this method did not use an AccessController. In
	 * version 1.1, an AccessController wrapper was incorrectly added to this
	 * method, causing a minor security flaw.
	 * <p>
	 * In version 1.1.1 this change was reverted; this method no longer uses an
	 * AccessController. User code wishing to obtain the context classloader must
	 * invoke this method via AccessController.doPrivileged if it needs support for
	 * that.
	 *
	 * @return the context classloader associated with the current thread, or null
	 *         if security doesn't allow it.
	 * @throws LogConfigurationException
	 *             if there was some weird error while attempting to get the context
	 *             classloader.
	 */
	 ...
	 * AccessController class. This means that java code running under a security
	 * manager that forbids access to ClassLoaders will still work if this class is
	 * given appropriate privileges, even when the caller doesn't have such
	 * privileges. Without using an AccessController, the the entire call stack must
	 * have the privilege before the call is allowed.
	 *
	 * @return the context classloader associated with the current thread, or null
	 *         if security doesn't allow it.
	 * @throws LogConfigurationException
	...
	/**
	 * Construct (if necessary) and return a <code>LogFactory</code> instance, using
	 ...
	 * <li>Use the properties file <code>commons-logging.properties</code> file, if
	...
	public static LogFactory getFactory() throws LogConfigurationException {
		...
		if (isDiagnosticsEnabled()) {
			logDiagnostic("[LOOKUP] LogFactory implementation requested for the first time for context classloader "
		...
		if (props != null) {
			...
			if (useTCCLStr != null) {
				...
				if (Boolean.valueOf(useTCCLStr).booleanValue() == false) {
					...
					// their
					// own logging implementations. It also means that it is up
					// to the
					// implementation whether to load library-specific config
					// files
		...
		if (factory == null) {
			...
			try {
				...
				if (isDiagnosticsEnabled()) {
					logDiagnostic("[LOOKUP] A security exception occurred while trying to create an"
							+ " instance of the custom factory class" + ": [" + trim(ex.getMessage())
							+ "]. Trying alternative implementations...");
				}
				// ignore
			}
	...
	/**
	 * Given a URL that refers to a .properties file, load that file. This is done
	 * under an AccessController so that this method will succeed when this jarfile
	 * is privileged but the caller is not. This method must therefore remain
	 * private to avoid security issues.
	 * <p>
	...
	/**
	 * Applets may run in an environment where accessing resources of a loader is a
	 * secure operation, but where the commons-logging library has explicitly been
	 * granted permission for that operation. In this case, we need to run the
	 * operation using an AccessController.
	 */
	 ...
	 * <p>
	 * This is just like ClassLoader.getResources except that the operation is done
	 * under an AccessController so that this method will succeed when this jarfile
	 * is privileged but the caller is not. This method must therefore remain
	 * private to avoid security issues.
	 * <p>
	 * If no instances are found, an Enumeration is returned whose hasMoreElements
	 * method returns false (ie an "empty" enumeration). If resources could not be
	 * listed for some reason, null is returned.
	 */
	 ...
	 * property can be read if JCL has been granted the appropriate security rights
	 * even if the calling code has not.
	 * <p>
	 * Take care not to expose the value returned by this method to the calling
	 * application in any way; otherwise the calling app can use that info to access
	 * data that should not be available to it.
	 */
	...
	/**
	 * Checks whether the supplied Throwable is one that needs to be re-thrown and
	 * ignores all others.
	 *
	 ...
	 * <code>LogFactory</code>. Diagnostic information is also logged.
	 * <p>
	 * <strong>Usage:</strong> to diagnose whether a classloader conflict is the
	 * cause of incompatibility. The test used is whether the class is assignable
	 * from the <code>LogFactory</code> class loaded by the class's classloader.
	 * 
		...
		if (logFactoryClass != null) {
			try {
				...
				if (logFactoryClassLoader == null) {
					...
					final Class<?> factoryFromCustomLoader = Class.forName("org.apache.commons.logging.LogFactory",
							false, logFactoryClassLoader);
				...
				// Running with diagnostics on should give information about the
				// classloaders used
	...
	/**
	 * Determines whether the user wants internal diagnostic output. If so, returns
	 * an appropriate writer object. Users can enable diagnostic output by setting
	 * the system property named {@link #DIAGNOSTICS_DEST_PROPERTY} to a filename,
	 * or the special values STDOUT or STDERR.
	 */
	...
	/**
	 * Generate useful diagnostics regarding the classloader tree for the specified
	 * class.
	 * <p>
	 * As an example, if the specified class was loaded via a webapp's classloader,
	 * then you may get the following output:
	 * 
	 * <pre>
	 ...
	 * <p>
	 * Note that this method is private; concrete subclasses of this class should
	 * not call it because the diagnosticPrefix string this method puts in front of
	 * all its messages is LogFactory@...., while subclasses should put
	 * SomeSubClass@...
	 * <p>
	...
	/**
	 * Logs diagnostic messages about the given classloader and it's hierarchy. The
	 * prefix is prepended to the message and is intended to make it easier to
	 * understand the logs.
	 * 
	 * @param prefix
	 * @param classLoader
	 ...
	 * takes 3 parameters.
	 * <p>
	 * This method would only ever be called in some rather odd situation. Note that
	 * this method is static, so overriding in a subclass doesn't have any effect
	 * unless this method is called from a method in that subclass. However this
	 * method only makes sense to use from the getFactory method, and as that is
	 * almost always invoked via LogFactory.getFactory, any custom definition in a
	 * subclass would be pointless. Only a class with a custom getFactory method,
	 * then invoked directly via CustomFactoryImpl.getFactory or similar would ever
	 * call this. Anyway, it's here just in case, though the "managed class loader"
	...
	/**
	 * Return a new instance of the specified <code>LogFactory</code> implementation
	 * class, loaded by the specified class loader. If that fails, try the class
	 * loader used to load this (abstract) LogFactory.
	 * <h2>ClassLoader conflicts</h2>
	 * <p>
	 * Note that there can be problems if the specified ClassLoader is not the same
	 * as the classloader that loaded this class, ie when loading a concrete
	 ...
	 * <p>
	 * The problem occurs when code running in the context classloader calls class X
	 * which was loaded via a parent classloader, and class X then calls
	 ...
	 * parent. When the code in this method finds some LogFactoryYYYY class in the
	 * child (context) classloader, and there also happens to be a LogFactory class
	 ...
	 * method cannot return the object as the desired type. Note that it doesn't
	 * matter if the LogFactory class in the child classloader is identical to the
	 ...
	 * occurs then throw an exception. The deployer of the application must ensure
	 ...
	 * the custom LogFactory subclass; that is ok as long as the only LogFactory
	 * class it can find to bind to is in the parent classloader.
	 *
	 * @param factoryClass
	 *            Fully qualified name of the <code>LogFactory</code> implementation
	 *            class
	 * @param classLoader
	...
	protected static LogFactory newFactory(final String factoryClass, final ClassLoader classLoader,
		...
		if (isDiagnosticsEnabled()) {
			logDiagnostic(
					"Created object " + objectId(result) + " to manage classloader " + objectId(contextClassLoader));
		}
		return (LogFactory) result;
	}

	/**
	 * Returns a string that uniquely identifies the specified object, including its
	 * class.
	 * <p>
	 ...
	 * instances that have been associated with the specified class loader (if any),
	...
	/**
	 * Release any internal references to previously created {@link LogFactory}
	 * instances, after calling the instance method <code>release()</code> on each
	 * of them. This is useful in environments like servlet containers, which
	 * implement application reloading by throwing away a ClassLoader. Dangling
	 * references to objects in that class loader would prevent garbage collection.
	 */
	 ...
	 * <code>LogFactory</code> you are using, the <code>Log</code> instance you are
	 * returned may or may not be local to the current application, and may or may
	 * not be returned again on a subsequent call with the same name argument.
	 *
	 * @param name
	 *            Logical name of the <code>Log</code> instance to be returned (the
	 *            meaning of this name is only known to the underlying logging
	...
	/**
	 * Set the configuration attribute with the specified name. Calling this with a
	 * <code>null</code> value is equivalent to calling
	 * <code>removeAttribute(name)</code>.
	 *
	 * @param name
	 *            Name of the attribute to set
	 * @param value
	 *            Value of the attribute to set, or <code>null</code> to remove any
