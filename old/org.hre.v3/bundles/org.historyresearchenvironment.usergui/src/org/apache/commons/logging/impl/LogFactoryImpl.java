 ...
 * <p>
 * This factory will remember previously created <code>Log</code> instances for
 * the same name, and will return them on repeated requests to the
...
public class LogFactoryImpl extends LogFactory {
	...
	/**
	 * The name (<code>org.apache.commons.logging.Log</code>) of the system property
	 * identifying our {@link Log} implementation class.
	 */
	...
	/**
	 * The deprecated system property used for backwards compatibility with old
	 * versions of JCL.
	 */
	protected static final String LOG_PROPERTY_OLD = "org.apache.commons.logging.log";

	/**
	 * The name (<code>org.apache.commons.logging.Log.allowFlawedContext</code>) of
	 * the system property which can be set true/false to determine system behaviour
	 * when a bad context-classloader is encountered. When set to false, a
	 * LogConfigurationException is thrown if LogFactoryImpl is loaded via a child
	 * classloader of the TCCL (this should never happen in sane systems).
	 *
	 ...
	 * of the system property which can be set true/false to determine system
	 * behaviour when a bad logging adapter class is encountered during logging
	 * discovery. When set to false, an exception will be thrown and the app will
	 * fail to start. When set to true, discovery will continue (though the user
	 ...
	 * of the system property which can be set true/false to determine system
	 * behaviour when a logging adapter class is encountered which has bound to the
	 * wrong Log class implementation. When set to false, an exception will be
	 * thrown and the app will fail to start. When set to true, discovery will
	 * continue (though the user might end up with a different logging
	...
	/**
	 * The names of classes that will be tried (in order) as logging adapters. Each
	 * class is expected to implement the Log interface, and to throw
	 * NoClassDefFound or ExceptionInInitializerError when loaded if the underlying
	 * logging library is not available. Any other error indicates that the
	 * underlying logging library is available but broken/unusable for some reason.
	 */
	private static final String[] classesToDiscover = { LOGGING_IMPL_LOG4J_LOGGER,
			"org.apache.commons.logging.impl.Jdk14Logger", "org.apache.commons.logging.impl.Jdk13LumberjackLogger",
			"org.apache.commons.logging.impl.SimpleLog" };

	/**
	 * Workaround for bug in Java1.2; in theory this method is not needed. See
	...
	/**
	 * Gets the context classloader. This method is a workaround for a java 1.2
	 * compiler bug.
	 * 
	 * @since 1.1
	 */
	 ...
	 *             if the current java security policy doesn't allow this class to
	...
	/**
	 * The one-argument constructor of the {@link org.apache.commons.logging.Log}
	 * implementation class that will be used to create new instances. This value is
	 * initialized by <code>getLogConstructor()</code>, and then returned
	 * repeatedly.
	 */
	...
	/**
	 * Attempts to load the given class, find a suitable constructor, and
	 * instantiate an instance of Log.
	 *
	 ...
	 * @param affectState
	 *            <code>true</code> if this object's state should be affected by
	 *            this method call, <code>false</code> otherwise.
	 * @return an instance of the given class, or null if the logging library
	 *         associated with the specified adapter is not available.
	 * @throws LogConfigurationException
	 *             if there was a serious error with configuration and the
	 *             handleFlawedDiscovery method decided this problem was fatal.
	 */
	...
	/**
	 * Attempts to create a Log instance for the given category name. Follows the
	 * discovery process described in the class javadoc.
	 *
	 * @param logCategory
	 *            the name of the log category
	 *
	 * @throws LogConfigurationException
	 *             if an error in discovery occurs, or if no adapter at all can be
	 *             instantiated
	 */
	...
	/**
	 * Checks system properties and the attribute map for a Log implementation
	 * specified by the user under the property names {@link #LOG_PROPERTY} or
	...
	private String findUserSpecifiedLogClassName() {
		...
		if (specifiedClass == null) { // @deprecated
			...
			try {
				...
				if (isDiagnosticsEnabled()) {
					logDiagnostic(
							"No access allowed to system property '" + LOG_PROPERTY_OLD + "' - " + e.getMessage());
				}
			}
		}
	...
	/**
	 * Return the classloader from which we should try to load the logging adapter
	 * classes.
	 * <p>
	 * This method usually returns the context classloader. However if it is
	 * discovered that the classloader which loaded this class is a child of the
	 * context classloader <i>and</i> the allowFlawedContext option has been set
	 * then the classloader which loaded this class is returned instead.
	 * <p>
	 * The only time when the classloader which loaded this class is a descendant
	 * (rather than the same as or an ancestor of the context classloader) is when
	 * an app has created custom classloaders but failed to correctly set the
	 * context classloader. This is a bug in the calling application; however we
	 * provide the option for JCL to simply generate a warning rather than fail
	 * outright.
	 *
	 */
	...
	/**
	 * Get the setting for the user-configurable behaviour specified by key. If
	 * nothing has explicitly been set, then return dflt.
	 */
	...
	/**
	 * Attempt to find an attribute (see method setAttribute) or a system property
	 * with the provided name and return its value.
	 * <p>
	 * The attributes associated with this object are checked before system
	 * properties in case someone has explicitly called setAttribute, or a
	...
	/**
	 * Return the fully qualified Java classname of the {@link Log} implementation
	 * we will be using.
	 *
	 * @deprecated Never invoked by this class; subclasses should not assume it will
	 *             be.
	 */
	@Deprecated
	...
	/**
	 * <p>
	 * Return the <code>Constructor</code> that can be called to instantiate new
	 * {@link org.apache.commons.logging.Log} instances.
	 * </p>
	 *
	 * <p>
	 * <strong>IMPLEMENTATION NOTE</strong> - Race conditions caused by calling this
	 * method from more than one thread are ignored, because the same
	 * <code>Constructor</code> instance will ultimately be derived in all
	 * circumstances.
	 * </p>
	 *
	 * @exception LogConfigurationException
	 ...
	 *            is a classloader (including the null classloader)
	 *
	 * @return c1 if it has c2 as an ancestor, c2 if it has c1 as an ancestor, and
	 *         null if neither is an ancestor of the other.
	 */
	...
	/**
	 * Generates an internal diagnostic logging of the discovery failure and then
	 * throws a <code>LogConfigurationException</code> that wraps the passed
	 * <code>Throwable</code>.
	 *
	 * @param logAdapterClassName
	 *            is the class name of the Log implementation that could not be
	 *            instantiated. Cannot be <code>null</code>.
	 *
	 * @param classLoader
	 ...
	 *             ALWAYS
	 */
	private void handleFlawedDiscovery(String logAdapterClassName, ClassLoader classLoader, // USED?
			Throwable discoveryFlaw) {

		if (isDiagnosticsEnabled()) {
			logDiagnostic("Could not instantiate Log '" + logAdapterClassName + "' -- "
					+ discoveryFlaw.getClass().getName() + ": " + discoveryFlaw.getLocalizedMessage());
			...
			if (discoveryFlaw instanceof InvocationTargetException) {
				...
				if (cause != null) {
					logDiagnostic("... InvocationTargetException: " + cause.getClass().getName() + ": "
	...
	/**
	 * Report a problem loading the log adapter, then either return (if the
	 * situation is considered recoverable) or throw a LogConfigurationException.
	 * <p>
	 * There are two possible reasons why we successfully loaded the specified log
	 * adapter class then failed to cast it to a Log object:
	 * <ol>
	 * <li>the specific class just doesn't implement the Log interface (user screwed
	 * up), or
	 ...
	 * </ol>
	 * <p>
	 * Here we try to figure out which case has occurred so we can give the user
	 * some reasonable feedback.
	 *
	 * @param badClassLoader
	 *            is the classloader we loaded the problem class from, ie it is
	 *            equivalent to badClass.getClassLoader().
	 *
	 * @param badClass
	 *            is a Class object with the desired name, but which does not
	 *            implement Log correctly.
	 *
	 * @throws LogConfigurationException
	 ...
	 * 
	 * @param messageBuffer
	 *            <code>StringBuffer</code> the message should be appended to, not
	 *            null
	 * @param name
	...
	/**
	 * Initialize a number of variables that control the behaviour of this class and
	 * that can be tweaked by the user. This is done when the first logger is
	 * created, not in the constructor of this class, because we need to give the
	 * user a chance to call method setAttribute in order to configure this object.
	 */
	 ...
	 * <p>
	 * Note that this method does not detect whether internal logging is enabled or
	 * not, nor where to output stuff if it is; that is all handled by the parent
	 * LogFactory class. This method just computes its own unique prefix for log
	 * messages.
	 */
	...
	/**
	 * Return <code>true</code> if <em>JDK 1.4 or later</em> logging is available.
	 * Also checks that the <code>Throwable</code> class supports
	...
	/**
	 * Utility method to check whether a particular logging library is present and
	 * available for use. Note that this does <i>not</i> affect the future behaviour
	 * of this class.
	 */
	...
	/**
	 * Output a diagnostic message to a user-specified destination (if the user has
	 ...
	 * application reloading by throwing away a ClassLoader. Dangling references to
	 ...
	 * <p>
	 * This method can be used to set logging configuration programmatically rather
	 * than via system properties. It can also be used in code running within a
	 * container (such as a webapp) to configure behaviour on a per-component level
	 * instead of globally as system properties would do. To use this method instead
	 * of a system property, call
	 * 
	 * <pre>
