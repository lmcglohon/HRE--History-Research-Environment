...
public class SimpleLog implements Log, Serializable {
	 ...
	 * <p>
	 * Any code that accesses this object should first obtain a lock on it, ie use
	 * synchronized(dateFormatter); this requirement was introduced in 1.1.1 to fix
	 * an existing thread safety bug (SimpleDateFormat.format is not thread-safe).
	 */
	...
	private static ClassLoader getContextClassLoader() {
		...
		try {
			...
			try {
				...
				/**
				 * InvocationTargetException is thrown by 'invoke' when the method being invoked
				 ...
				 *
				 * In the first case (not related), we want to ignore and keep going. We cannot
				 * help but also ignore the second with the logic below, but other calls
				 * elsewhere (to obtain a class loader) will trigger this exception where we can
				 * make a distinction.
				 */
	 ...
	 * <p>
	 * This allows expensive operations such as <code>String</code> concatenation to
	 * be avoided when the message will be ignored by the logger.
	 */
	@Override
	 ...
	 * <p>
	 * This method assembles the message and then calls <code>write()</code> to
	 ...
	 * <code>StringBuffer</code> to the appropriate output destination. The default
