...
public class DBT_iv extends DBTFile {
	...
	public byte[] readBytes(byte[] input) throws IOException, xBaseJException {
		...
		final String sPos = new String(input, 0, 10).trim();
		if (sPos.length() == 0) {
			return null;
		}
		final long lPos = Long.parseLong(sPos.trim());
		if (lPos == 0) {
			return null;
		}
		...
		final int lastind = Util.x86(file.readInt());

		if (lastind != LAST_IND) {
			throw new xBaseJException("Unexpected encounter in read text file");
		}

		final int size = Util.x86(file.readInt());

		orisize = size - 8;

		final byte work_buffer[] = new byte[orisize + 1];
	...
	public byte[] write(String value, int originalSize, boolean write, byte originalPos[])
		...
		while (true) {
			try {
				final long longnextstop = next_stop;
		...
		final String returnString = new String(Long.toString(next_stop));
