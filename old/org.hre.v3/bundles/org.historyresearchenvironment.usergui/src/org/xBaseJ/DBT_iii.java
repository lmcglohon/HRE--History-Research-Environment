...
public class DBT_iii extends DBTFile {
	...
	public byte[] readBytes(byte[] input) throws IOException, xBaseJException {
		...
		final long lPos = Long.parseLong(sPos);
		if (lPos == 0) {
			return null;
		}
	...
	public byte[] write(String value, int originalSize, boolean write, byte originalPos[])
		...
		if (value.length() == 0) {
			final byte breturn[] = { BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO, BYTEZERO,
					BYTEZERO, BYTEZERO };
			return breturn;
		}
		...
		for (pos = 0; pos < value.length(); pos += memoBlockSize) {
			...
			if ((pos + memoBlockSize) > value.length()) {
				...
				try {
					b = value.substring(pos, (pos + memoBlockSize)).getBytes(DBF.encodedType);
				} catch (final UnsupportedEncodingException UEE) {
