...
import org.xBaseJ.fields.IntegerField;
import org.xBaseJ.fields.LogicalField;
import org.xBaseJ.fields.MemoField;
import org.xBaseJ.fields.NumField;
import org.xBaseJ.fields.OleField;
...
public class DBF extends Object {
	...
	public static final String xBaseJVersion = "20140310M";
	...
	protected short fldcount_calc = 0;
	 ...
	 *             Java error caused by called methods, most likely trying to create
	 *             on a remote system
	 */
	...
	public void addField(ArrayList<Field> aField) throws xBaseJException, IOException {
		final Field[] flds = new Field[aField.size()];
	...
	public void addField(Field aField) throws xBaseJException, IOException {
		...
		final Field bField[] = new Field[1];
		bField[0] = aField;
		addField(bField);
	...
	public void addField(Field aField[]) throws xBaseJException, IOException {
		...
		final String ignoreDBFLength = Util.getxBaseJProperty("ignoreDBFLengthCheck");
		...
		if (createTemp) {
			...
			final File f = File.createTempFile("org.xBaseJ", ffile.getName());
		...
		if (tempDBF.dbtobj != null) {
			...
			if ((newName != null) && (newName.length() > 4)) {
				final String tempMDXFilename = newName.substring(0, newName.length() - 4) + ".mdx";
				final File tempMDXFile = new File(tempMDXFilename);
	...
	public void copyTo(String newname) throws IOException {
		...
		final FileChannel srcChannel = new FileInputStream(dosname).getChannel();
		@SuppressWarnings("resource")
		final FileChannel dstChannel = new FileOutputStream(newname).getChannel();
	...
	protected void createDBF(String DBFname, DBFTypes format, boolean destroy)
		...
		final FileOutputStream tFOS = new FileOutputStream(ffile);
		tFOS.close();
		...
		final boolean memoExists = ((format == DBFTypes.DBASEIII_WITH_MEMO) || (format == DBFTypes.DBASEIV_WITH_MEMO)
	...
	public boolean find(String keyString, boolean lock) throws xBaseJException, IOException {
		...
		final int r = jNDX.find_entry(keyString);
		if (r < 1) {
	...
	public boolean find(String keyString, int recno, boolean lock) throws xBaseJException, IOException {
		...
		final int r = jNDX.find_entry(keyString, recno);
		if (r < 1) {
			throw new xBaseJException("Record not found");
		}
	 ...
	 * 
	 * @throws xBaseJException
	 *             org.xBaseJ Index not opened or not part of the database eof - end
	 *             of file
	 * @throws IOException
	 *             Java error caused by called methods
	 */
	 ...
	 * 
	 * @throws xBaseJException
	 *             org.xBaseJ Index not opened or not part of the database tof - top
	 *             of file
	 * @throws IOException
	 *             Java error caused by called methods
	 */
	 ...
	 *            Field number
	 * @throws xBaseJException
	 *             usually occurs when Field number is less than 1 or greater than
	 ...
	 * @throws xBaseJException
	 *             passed an negative number, 0 or value greater than the number of
	 *             records in database
	 * @throws IOException
	 *             Java error caused by called methods
	 */
	...
	public void gotoRecord(int recno, boolean lock) throws xBaseJException, IOException {
		...
		for (i = 0; i < fldcount; i++) {
			...
			tField.read();
			// MER
			// System.out.println(tField.getName() + ": " + tField.get());
		}
	...
	public void lock() throws IOException, xBaseJException {
		...
		final long thisWait = fileLockWait / 5;
	...
	public void lockRecord(int recno) throws IOException, xBaseJException {
		...
		unlockRecord();
		final long calcpos = (offset + (lrecl * (recno - 1)));
	...
	protected void openDBF(String DBFname) throws IOException, xBaseJException {
		...
		final String fptname = new String(dosname.substring(0, dosname.length() - 3) + "fpt");
		final File f = new File(fptname);
		if (f.exists() && !f.isDirectory()) {
			version = DBFTypes.FOXPRO_WITH_MEMO;
			System.out.println("Version corrected to " + DBFTypes.FOXPRO_WITH_MEMO);
		}
		...
		if ((version != DBFTypes.DBASEIII) && (version != DBFTypes.DBASEIII_WITH_MEMO) && (version != DBFTypes.DBASEIV)
			...
			final String mismatch = Util.getxBaseJProperty("ignoreVersionMismatch").toLowerCase();
		...
		// MER
		if (fldcount != fldcount_calc) {
			System.out.println("Field count corrected from " + fldcount + " to " + fldcount_calc);
			fldcount = fldcount_calc;
		}
		...
		if (MDX_exist == 1) {
			...
			} catch (final xBaseJException xbe) {
				final String missing = Util.getxBaseJProperty("ignoreMissingMDX").toLowerCase();
	...
	public void pack() throws xBaseJException, IOException, SecurityException, CloneNotSupportedException {
		final Field Fields[] = new Field[fldcount];
		...
		final File f = File.createTempFile("tempxbase", "tmp");
		final String tempname = f.getAbsolutePath();

		final DBF tempDBF = new DBF(tempname, version, true);
	...
	protected void read_dbhead() throws IOException {
		...
		file.seek(0);
		final byte fileVersion = file.readByte();
	...
	protected Field read_Field_header() throws IOException, xBaseJException {
		...
		case FloatField.type:
			...
			break;
		case OleField.type:
			tField = new OleField(name, buffer, dbtobj);
			fldcount_calc++;
			break;
		...
		case IntegerField.type:
			tField = new IntegerField(name, buffer);
			fldcount_calc++;
			break;
		default:
			System.out.println("Unknown Field type '" + type + "' for " + name);
			// throw new xBaseJException("Unknown Field type '" + type +
			// "' for "
			// + name);
		} /* endswitch */
	...
	public void renameTo(String newname) throws IOException {
		file.close();
		final File n = new File(newname);
		final boolean b = ffile.renameTo(n); // 20091012_rth - begin
		if (!b) {
			copyTo(newname);
			ffile.delete();
		} // 20091012_rth - end
	...
	protected void seek(long recno) throws IOException {
		...
		final long calcpos = (offset + (lrecl * recno));
		file.seek(calcpos);
	}
	 ...
	 * 
	 * @param lock
	 *            - boolean lock indicator - locks the entire file during the write
	...
	protected void write_Field_header(Field tField) throws IOException, xBaseJException {
		...
		final int nameLength = tField.getName().length();
		int i = 0;
		byte b[];
		try {
