...
public abstract class DBTFile extends Object {
	...
	public DBTFile(DBF iDBF, boolean readonly, DBFTypes foxproWithMemo)
		...
		database = iDBF;
		final String name = iDBF.getName();

		String tname;
		final String ext = Util.getxBaseJProperty("memoFileExtension");
		if (ext.length() > 0) {
			tname = new String(name.substring(0, name.length() - 3) + ext);
			extension = ext;
		} else if (foxproWithMemo == DBFTypes.FOXPRO_WITH_MEMO) {
			extension = "fpt";
			...
			thefile = new File(tname);
			if (!thefile.exists() || !thefile.isFile()) {
				final String dtname = new String(name.substring(0, name.length() - 3) + "fpt");
	...
	public DBTFile(DBF iDBF, String name, boolean destroy, DBFTypes type) throws IOException, xBaseJException {
		...
		final FileOutputStream tFOS = new FileOutputStream(thefile);
		tFOS.close();
	...
	public void rename(String name) throws IOException {
		...
		final String tname = new String(name.substring(0, name.length() - 1) + "t");
		file.close();
