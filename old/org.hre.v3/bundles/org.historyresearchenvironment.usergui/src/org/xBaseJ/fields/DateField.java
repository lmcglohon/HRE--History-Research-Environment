...
public class DateField extends Field {
	...
	public Object clone() throws CloneNotSupportedException {
		final DateField tField = (DateField) super.clone();
		tField.Name = Name;
		tField.Length = 8;
		return tField;
	}
	...
	public int compareTo(Calendar compareThis) throws xBaseJException {
		final DateField compareDateField = new DateField();
	...
	public Calendar getCalendar() throws xBaseJException {
		final Calendar getter = Calendar.getInstance();
		...
		for (i = 0; i < 8; i++) {
			if (Character.isDigit(inValue.charAt(i)) == false) {
				throw new xBaseJException(
						"Invalid format for date Field, " + inValue + " non numeric at position " + i);
			}
		}

		final int yea = Integer.parseInt(inValue.substring(0, 4));
		final int mo = Integer.parseInt(inValue.substring(4, 6));
		if ((mo < 1) || (mo > 12)) {
			throw new xBaseJException("Invalid format for date Field (month) " + inValue);
		}
		final int da = Integer.parseInt(inValue.substring(6, 8));
		if (da < 1) {
			throw new xBaseJException("Invalid format for date Field (day) " + inValue);
		}

		final int month[] = { 0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
