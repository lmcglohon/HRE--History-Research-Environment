 ...
 *          USA
 *
 *          Change History Date Developer Desc User: Dmitry Berezovsky (corvis)
 *          Date: 1/9/12 Time: 12:25 AM
 * 
 *          20110119 Joe McVerry (jrm) Added static field type and CurrencyField
 *          class.
 */
public class CurrencyField extends Field {
	...
	private final BigDecimal currencyFromByteArray(byte[] bytes) {
		final long d = ((long) (bytes[7]) << 56) | (/* long cast needed or shift done modulo 32 */
	...
	private final byte[] currencyToByteArray(BigDecimal currencyValue) {
		...
		final long v = currencyValue.multiply(new BigDecimal(10000L)).longValue();
		bytes[0] = (byte) v;
		bytes[1] = (byte) (v >> 8);
		bytes[2] = (byte) (v >> 16);
		bytes[3] = (byte) (v >> 24);
	...
	@Override
	public String get() {
		final BigDecimal val = currencyFromByteArray(super.getBytes());
