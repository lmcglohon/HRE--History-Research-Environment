...
public class dbfShow extends Frame implements ActionListener, WindowListener, AdjustmentListener, ItemListener {
	...
	public static void addComponent(Container container, Component component, int gridx, int gridy, int gridwidth,
			int gridheight, int fill, int anchor) throws AWTException {
	...
	memoDialog md;
	MenuItem opener, packer, quiter, firstRecord, nextRecord, prevRecord, lastRecord, addRecord, updateRecord,
			clearRecord;
	Label l;
	TextField t;
	Field f;
	...
	public dbfShow(String title, String fname) throws Exception {
		...
		if ((fname == null) || (fname.length() == 0)) {
			final FileDialog fd = new FileDialog(this, "dbfShow", FileDialog.LOAD);
			fd.setFile("*.DBF");
			fd.pack();
			fd.setVisible(true);
			final String DBFname = fd.getFile();
		...
		mb.add(file);
		final Menu record = new Menu("Record");
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (event.getSource() == opener) {
			...
			final String dbname = new String(dirname + DBFname);
			try {
				setupDBFields(dbname);
	...
	public void setupDBFields(String dbname) throws Exception {
		...
		final Dimension dimView = sp.getSize();
	...
	public void updateRec() {
		...
		try {
			setFields();
			db.update();
			final boolean b = delCB.getState();
			if (db.deleted() && !b) {
				db.undelete();
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (event.getSource() == Okay) {
			try {
				...
				f1.put(text.getText());
				dispose();
			} catch (final xBaseJException e1) {
				System.out.println(e1);
			}
		}
	}
