...
public class MDX extends Index {
	...
	public MDX(MDXFile ifile, DBF iDBF, short ipos) throws xBaseJException, IOException {
		...
		while (reading > 0) {
			if (topNode == null) {
				...
				final MNode llNode = new MNode(mfile, key_per_Node, key_length, keyType, Index_record, false);
	...
	public MDX(String iname, String NDXString, DBF iDBF, MDXFile ifile, TagDescriptor inTagDesc, short pos,
...

	{
		...
		final StringTokenizer strtok = new StringTokenizer(NDXString, "+");
	...
	private void splitNode(MNode aNode, int savepos) throws xBaseJException, IOException {
		...
		if (savepos > i) {
			...
			aNode.write();
			left = aNode.get_record_number();
			update_entry((MNode) aNode.get_prev(), aNode.get_key_value(), left, right);
			if (aNode.get_prev() == null) {
				aNode.set_prev(topNode);
				topNode.set_next(aNode);
			}
	...
	private int update_entry(MNode aNode, NodeKey NDXkey, int recno, int oldrecno) throws IOException, xBaseJException {
		...
		if ((topNode == null) || (topNode.get_record_number() == 0)) { /*
			...
			topNode.set_record_number(tagHead.top_Node);
		...
		if (aNode == null) { /* work to split the top Node */
			final MNode newNode = new MNode(mfile, key_per_Node, key_length, keyType, mfile.anchor.get_nextavailable(),
					true);
			...
			newNode.set_record_number(tagHead.top_Node);
			newNode.write();
