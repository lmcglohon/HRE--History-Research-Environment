...
public class NDX extends Index {
	...
	public NDX(String filename, DBF indatabase, char readonly) throws IOException, xBaseJException {
		...
		final StringTokenizer strtok = new StringTokenizer(stringKey, "+ ");
		...
		while (reading > 0) {
			if (topNode == null) {
				...
				final Node llNode = new Node(nfile, key_per_Node, key_length, keyType, Index_record, false);
			...
			if (reading > 0) {// /* if reading is zero we're still reading Nodes, when < 0 then read
								// * a leaf */
	 ...
	 * @param NDXString
	 *            database Fields that define the Index, join Fields with "+" do not
	 *            use spaces
	 * @param indatabase
	...
	public NDX(String name, String NDXString, DBF indatabase, boolean destroy, boolean unique)
			throws xBaseJException, IOException {
		int len;
		String tempch1;
		char key_type = ' ';
		final StringTokenizer strtok = new StringTokenizer(NDXString.toUpperCase(), "+");
		...
		while (strtok.hasMoreElements()) {
			...
			final Field Field = database.getField(tempch1);
			type = Field.getType();
			if (type == 'M') {
	...
	public void bIndex() throws xBaseJException, IOException {
		int i;
		final int reccount = database.getRecordCount();
		NodeKey lastkey;
	...
	public int find_entry(NodeKey findKey, Node aNode, int findrec) throws xBaseJException, IOException {
		...
		for (aNode.set_pos(0); aNode.get_pos() < until; aNode.pos_up()) {
			leaf = aNode.get_lower_level();
			rec = aNode.get_key_record_number();
			if (aNode.get_pos() < aNode.get_keys_in_this_Node()) { /* leafs make us do this */
	...
	private int reIndexWork(BinaryTree bt, int level) throws IOException, xBaseJException {
		...
		btLoop: while (true) {
			if ((pos == key_per_Node) || (bt == null)) {
				...
				if ((bt != null) || (tree2 != null)) { // if bt not null more keys to add and to upper node if tree2 not
														// nuul then upper node needs the last node
					if (tree2 == null) {
						topNode = workNode;
	...
	private void splitNode(Node aNode, int savepos) throws xBaseJException, IOException {
		...
		if (savepos > i) {
			...
			if (aNode.get_prev() != null) {
				bNode.set_pos(i - 1);
				update_entry(aNode.get_prev(), bNode.get_key_value(), aNode.get_record_number(), right, 0);
			}
			aNode.set_pos(0);
			bNode.set_pos(i);
			for (k = 0; k <= j; k++) {
		...
		} else { /* create new second half and use the first half to put new key in */
			...
			left = aNode.get_record_number();
			if (aNode.get_prev() != null) {
				update_entry(aNode.get_prev(), aNode.get_key_value(), left, right, 0);
			} else {
				bNode.set_pos(j - 1);
	...
	private int update_entry(Node aNode, NodeKey NDXkey, int leftleaf, int rightleaf, int recno)
		...
		if (topNode == null) { /*
								 * we don't have any Index area yet so we must be adding the first record
								 */
			topNode = new Node(nfile, key_per_Node, key_length, keyType, next_available, false);
			workNode = topNode;
			topNode.set_pos(0);
		 ...
		 */
		/*
		 * for all other conditions if passed a leaf or a Node one of the two (leaf no
		 * or rec no)
		 */
		/* will be zero */
		...
		if (savepos < (aNode.get_keys_in_this_Node())) {
			...
			/*
			 * first move the record number of that trailing record indicator dbase III
			 * quirk
			 */
			int ptr, recn, i;
			NodeKey buf;
