 ...
 * <p>
 * Copyright 1997-2014 - American Coders, LTD - Raleigh NC USA
 * <p>
 * All rights reserved
 * <p>
...
public class NodeFloat {
	...
	public NodeFloat(byte invalue[]) {
		...
		final byte neg1 = (byte) 0xd1;
		final byte neg2 = (byte) 0xa9;
	...
	public NodeFloat(double invalue) {
		...
		final int bLine[] = { 0, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90 };
		int start = 0;

		final boolean neg = (invalue < 0.0);
		if (neg) {
			invalue *= -1.0;
		}

		final Double d = new Double(invalue);
		String s = d.toString();

		final int epos = (s.indexOf('e') < 0 ? 0 : s.indexOf('e') + 1) + (s.indexOf('E') < 0 ? 0 : s.indexOf('E') + 1);
		final int decpos = s.indexOf('.');
		String s2;
		int t;
		if (epos > 0) {
	...
	public int compareTo(NodeFloat nf2) {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (value[i] < 0) // reverse all logic for bytes whose first nibble is 8 or 9
			{
			...
			if (nf2.value[i] < 0) // reverse all logic for bytes whose first nibble is 8 or 9
			{
