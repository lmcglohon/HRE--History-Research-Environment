...
public class NodeKey {
	...
	public int compareKey(NodeKey keyCompareTo) // throws new xBaseJException
	{
		...
		if (type == 'F') {
			final NodeFloat nf = (NodeFloat) key;
			final NodeFloat nft = (NodeFloat) keyCompareTo.key;
			return nf.compareTo(nft);
		}
		final Double d = (Double) key;

		final double d2 = d.doubleValue() - keyCompareTo.toDouble();
		if (d2 < 0.0) {
			return -1;
		}
		if (d2 > 0.0) {
	...
	public String rebuildString(String inString) {
		final char a[] = new char[inString.length()];
	...
	public NodeFloat toNodeFloat() {
		if (type == 'F') {
			final NodeFloat f = (NodeFloat) key;
			return f;
		}
		return null;
	}
