...
public class MDXFile {
	...
	public MDXFile(String Name, DBF inDBF, boolean destroy) throws IOException {
		try {
			...
			final FileOutputStream tFOS = new FileOutputStream(file);
			tFOS.close();
	...
	private void closeFile(RandomAccessFile f) {
		if (f != null) {
			try {
				f.close();
			} catch (final IOException nothingToDo) {
			}
		}
	}
	...
	public MDX createTag(String Name, String Index, boolean unique) throws IOException, xBaseJException {
		...
		final short i = (short) (anchor.getIndexes() + 1);
		tags[i - 1] = new TagDescriptor(this, i, Name);
		final MDX newMDX = new MDX(Name, Index, database, this, tags[i - 1], i, unique);
		anchor.setIndexes(i);
		anchor.write();
		MDXes[i - 1] = newMDX;
	...
	public void reIndex() throws IOException, xBaseJException {
		final short oldIndexCount = anchor.getIndexes();
		short i;
		raFile.close();
		file.delete();
		raFile = new RandomAccessFile(file, "rw");
