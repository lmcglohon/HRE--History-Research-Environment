...
public class DBT_fpt extends DBTFile {
	...
	public DBT_fpt(DBF iDBF, boolean readOnly) throws IOException, xBaseJException {
		...
		// System.out.println("Memo block size: " + memoBlockSize);
	}
	...
	public byte[] readBytes(byte[] input) throws IOException, xBaseJException {
		...
		int i;
		int j;
		final Vector<Integer> iv = new Vector<Integer>(4);

		// System.out.print("Input to readBytes: ");
		// MER
		// for (i = 0; i < 10; i++) {
		for (i = 0; i < 4; i++) {
			...
			j = (input[i] < 0 ? input[i] + 256 : input[i]);
		...
		// String sPos = new String(input, 0, 4);
		...
		final int lPos = iv.elementAt(0)
				+ (256 * (iv.elementAt(1) + (256 * (iv.elementAt(2) + (256 * (iv.elementAt(3)))))));
		...
		final byte work_buffer[] = new byte[orisize];
	...
	public void rename(String name) throws IOException {
		...
		final String tname = new String(name.substring(0, name.length() - 3) + "fpt");
		file.close();
		final File nfile = new File(tname);
		nfile.delete();
	...
	public byte[] write(byte inBytes[], int originalSize, boolean write, byte originalPos[])
		...
		if (inBytes.length == 0) {
			final byte breturn[] = { BYTESPACE, BYTESPACE, BYTESPACE, BYTESPACE, BYTESPACE, BYTESPACE, BYTESPACE,
					BYTESPACE, BYTESPACE, BYTESPACE };
			return breturn;
		}
		...
		final String returnString = new String(Long.toString(startPos));
	...
	public byte[] write(String value, int originalSize, boolean write, byte originalPos[])
			throws IOException, xBaseJException {
		try {
			return write(value.getBytes(DBF.encodedType), originalSize, write, originalPos);
