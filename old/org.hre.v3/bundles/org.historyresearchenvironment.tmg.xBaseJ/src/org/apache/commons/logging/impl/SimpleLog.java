...
public class SimpleLog implements Log, Serializable {
	 ...
	 * <p>
	 * Any code that accesses this object should first obtain a lock on it, ie
	 * use synchronized(dateFormatter); this requirement was introduced in 1.1.1
	 * to fix an existing thread safety bug (SimpleDateFormat.format is not
	 * thread-safe).
	 */
	...
	protected void log(int type, Object message, Throwable t) {
		...
		if (showShortName) {
			if (shortLogName == null) {
				...
				final String slName = logName.substring(logName
						.lastIndexOf(".") + 1);
	 ...
	 *
	 * @param buffer
	 *            A <code>StringBuffer</code> containing the accumulated text to
	 *            be logged
	 */
	...
	private static ClassLoader getContextClassLoader() {
		...
		try {
			...
			final Method method = Thread.class.getMethod(
					"getContextClassLoader", (Class[]) null);

			// Get the thread context class loader (if there is one)
			try {
				classLoader = (ClassLoader) method.invoke(
						Thread.currentThread(), (Class[]) null);
