...
public abstract class LogFactory {
	 ...
	 * property used to identify the LogFactory implementation class name. This
	 * can be used as a system property, or as an entry in a configuration
	 * properties file.
	 */
	...
	/**
	 * The name (<code>commons-logging.properties</code>) of the properties file
	 * to search for.
	 */
	 ...
	 * <p>
	 * A typical use case is to allow a custom Hashtable implementation using
	 * weak references to be substituted. This will allow classloaders to be
	 * garbage collected without the need to release them (on 1.3+ JVMs only, of
	 * course ;).
	 */
	 ...
	 * <code>getClassLoader</code> returns <code>null</code>. This can happen
	 * when:
	 * <ul>
	 ...
	 *
	 * @param classLoader
	 *            should be the current context classloader. Note that this can
	 *            be null under some circumstances; this is ok.
	 * @param factory
	...
	protected static Object createFactory(String factoryClass,
		...
		try {
			 ...
			 * LogFactory.class.getClassLoader().load(name), ie we ignore the
			 * classloader parameter the caller passed, and fall back to trying
			 * the classloader associated with this class. See the javadoc for
			 * the newFactory method for more info on the consequences of this.
			 * 
			...
			// to be generated/caught & recast properly.
			if (isDiagnosticsEnabled()) {
				logDiagnostic("Unable to load factory class via classloader "
						+ objectId(classLoader)
	 ...
	 * "weak references", allowing a custom Hashtable class to be used which
	 * uses only weak references to its keys. Using weak references can fix
	 * memory leaks on webapp unload in some cases (though not all). Version 1.1
	 * of Java does not support weak references, so we must dynamically
	 * determine which we are using. And just for fun, this code also supports
	 * the ability for a system property to specify an arbitrary Hashtable
	 * implementation name.
	 * <p>
	 * Note that the correct way to ensure no memory leaks occur is to ensure
	 * that LogFactory.release(contextClassLoader) is called whenever a webapp
	 * is undeployed.
	 */
	...
	private static final Hashtable<ClassLoader, LogFactory> createFactoryStore() {
		...
		try {
			storeImplementationClass = getSystemProperty(
					HASHTABLE_IMPLEMENTATION_PROPERTY, null);
		...
		try {
			...
			result = (Hashtable<ClassLoader, LogFactory>) implementationClass
					.newInstance();
		} catch (Throwable t) {
			...
			if (!WEAK_HASHTABLE_CLASSNAME.equals(storeImplementationClass)) {
				...
				if (isDiagnosticsEnabled()) {
					...
					System.err
							.println("[ERROR] LogFactory: Load of custom hashtable failed");
				}
			}
		}
		if (result == null) {
	 ...
	 * <p>
	 * Note that no internal logging is done within this method because this
	 * method is called every time LogFactory.getLogger() is called, and we
	 * don't want too much output generated here.
	 *
	 * @throws LogConfigurationException
	 *             if a suitable class loader cannot be identified.
	 * @return the thread's context classloader or {@code null} if the java
	 *         security policy forbids access to the context classloader from
	 *         one of the classes in the current call stack.
	 * @since 1.1
	 */
	 ...
	 *         previously been created, or null if this is the first time we
	 *         have seen this particular classloader.
	 */
	 ...
	 * AccessController. User code wishing to obtain the context classloader
	 * must invoke this method via AccessController.doPrivileged if it needs
	 * support for that.
	 *
	 * @return the context classloader associated with the current thread, or
	 *         null if security doesn't allow it.
	 * @throws LogConfigurationException
	 *             if there was some weird error while attempting to get the
	 *             context classloader.
	 */
	 ...
	 * security manager that forbids access to ClassLoaders will still work if
	 * this class is given appropriate privileges, even when the caller doesn't
	 * have such privileges. Without using an AccessController, the the entire
	 * call stack must have the privilege before the call is allowed.
	 *
	 * @return the context classloader associated with the current thread, or
	 *         null if security doesn't allow it.
	 * @throws LogConfigurationException
	...
	public static LogFactory getFactory() throws LogConfigurationException {
		...
		Properties props = getConfigurationFile(contextClassLoader,
				FACTORY_PROPERTIES);
		...
		if (isDiagnosticsEnabled()) {
			logDiagnostic("[LOOKUP] Looking for system property ["
					+ FACTORY_PROPERTY
		...
		try {
			...
			if (factoryClass != null) {
				...
				if (isDiagnosticsEnabled()) {
					logDiagnostic("[LOOKUP] No system property ["
							+ FACTORY_PROPERTY + "] defined.");
				}
			}
		} catch (SecurityException e) {
			if (isDiagnosticsEnabled()) {
			...
			try {
				...
				if (is != null) {
					...
					if (factoryClassName != null
						...
						factory = newFactory(factoryClassName, baseClassLoader,
								contextClassLoader);
					}
				} else {
					// is == null
					if (isDiagnosticsEnabled()) {
						logDiagnostic("[LOOKUP] No resource file with name '"
								+ SERVICE_ID + "' found.");
					}
				}
			} catch (Exception ex) {
		...
		if (factory == null) {
			if (props != null) {
				if (isDiagnosticsEnabled()) {
					logDiagnostic("[LOOKUP] Looking in properties file for entry with key '"
							+ FACTORY_PROPERTY
							+ "' to define the LogFactory subclass to use...");
				}
				...
				if (factoryClass != null) {
					if (isDiagnosticsEnabled()) {
						logDiagnostic("[LOOKUP] Properties file specifies LogFactory subclass '"
								+ factoryClass + "'");
					}
					factory = newFactory(factoryClass, baseClassLoader,
							contextClassLoader);
				...
				if (isDiagnosticsEnabled()) {
					logDiagnostic("[LOOKUP] No properties file available to determine"
	 ...
	 * is a secure operation, but where the commons-logging library has
	 * explicitly been granted permission for that operation. In this case, we
	 * need to run the operation using an AccessController.
	 */
	 ...
	 * cause of incompatibility. The test used is whether the class is
	 * assignable from the <code>LogFactory</code> class loaded by the class's
	 * classloader.
	 * 
	...
	private static boolean implementsLogFactory(Class<?> logFactoryClass) {
		...
		if (logFactoryClass != null) {
			try {
				ClassLoader logFactoryClassLoader = logFactoryClass
						.getClassLoader();
				if (logFactoryClassLoader == null) {
					logDiagnostic("[CUSTOM LOG FACTORY] was loaded by the boot classloader");
				} else {
					logHierarchy("[CUSTOM LOG FACTORY] ", logFactoryClassLoader);
					Class<?> factoryFromCustomLoader = Class.forName(
							"org.apache.commons.logging.LogFactory", false,
	 ...
	 * fails, try the class loader used to load this (abstract) LogFactory. <h2>
	 * ClassLoader conflicts</h2>
	 * <p>
	...
	protected static LogFactory newFactory(final String factoryClass,
		...
		if (isDiagnosticsEnabled()) {
			logDiagnostic("Created object " + objectId(result)
