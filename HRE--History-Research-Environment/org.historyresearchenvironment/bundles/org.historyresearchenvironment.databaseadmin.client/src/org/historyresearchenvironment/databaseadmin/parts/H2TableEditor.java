...
public class H2TableEditor {
		...
		btnInsert.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				try {
					...
					eventBroker.post(
							org.historyresearchenvironment.dataaccess.HreDataAccessConstants.TABLENAME_UPDATE_TOPIC,
							tableName);
	...
	private void createLines() throws SQLException {
		...
		for (int i = 0; i < count; i++) {
			...
			case HreDataAccessConstants.BIGINT:
				...
				lineList.add(text);
				break;
			case HreDataAccessConstants.BLOB:
				text = createFieldLine(compositeFields, i);
				...
				lineList.add(text);
				break;
			case HreDataAccessConstants.BOOLEAN:
				...
				break;
			case HreDataAccessConstants.CLOB:
				text = createFieldLine(compositeFields, i);
				columns.get(i).setValue(row.get(i));
				...
				lineList.add(text);
				break;
			case HreDataAccessConstants.DOUBLE:
				text = createFieldLine(compositeFields, i);
				text.addVerifyListener(new NumericVerifyListener());
				columns.get(i).setValue(row.get(i));
				...
				lineList.add(text);
				break;
			case HreDataAccessConstants.INTEGER:
				text = createFieldLine(compositeFields, i);
				...
				lineList.add(text);
				break;
			case HreDataAccessConstants.SMALLINT:
				text = createFieldLine(compositeFields, i);
				...
				lineList.add(text);
				break;
			case HreDataAccessConstants.TIMESTAMP:
				final Label label = new Label(compositeFields, SWT.NONE);
				label.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1));
				label.setText(columns.get(i).getName());
				...
				break;
			case HreDataAccessConstants.VARBINARY:
				text = createFieldLine(compositeFields, i);
				text.addVerifyListener(new HexVerifyListener());
				text.addFocusListener(new HexFocusListener());
				...
				lineList.add(text);
				break;
			case HreDataAccessConstants.VARCHAR:
				text = createFieldLine(compositeFields, i);
