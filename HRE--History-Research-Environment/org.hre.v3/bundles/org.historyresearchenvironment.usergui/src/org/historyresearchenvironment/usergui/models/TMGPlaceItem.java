 ...
 *
 */
public class TMGPlaceItem extends AbstractHreItem {
	private int recNo;
	private int uid;
	private int type;
	 ...
	 * @param xValue
	 */
	public TMGPlaceItem(int recNo, String[] placeParts) {
		super();
		this.recNo = recNo;
	...
	public String toString() {
		...
		for (int i = 0; i < placeParts.length; i++) {
			if (placeParts[i].length() > 0) {
				if (first) {
					first = false;
				} else {
					sb.append(", ");
				}
				sb.append(placeParts[i]);
			}
		}
		return sb.toString();
