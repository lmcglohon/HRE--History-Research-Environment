...
public class DBF extends Object {
	...
	public DBF(String DBFname, DBFTypes format, boolean destroy,
			String inEncodeType) throws xBaseJException, IOException,
			SecurityException {
		setEncodingType(inEncodeType);
		createDBF(DBFname, format, destroy);
	}
	...
	public void addField(ArrayList<Field> aField) throws xBaseJException,
			IOException {
	...
	public void addField(Field aField[]) throws xBaseJException, IOException {
		...
		if (tempDBF.dbtobj != null) {
			...
			if (newName != null && newName.length() > 4) {
				String tempMDXFilename = newName.substring(0,
						newName.length() - 4) + ".mdx";
	...
	protected void createDBF(String DBFname, DBFTypes format, boolean destroy)
		...
		if (format != DBFTypes.DBASEIII && format != DBFTypes.DBASEIV
		...
		boolean memoExists = (format == DBFTypes.DBASEIII_WITH_MEMO
	...
	protected void db_offset(DBFTypes format, boolean memoPresent) {
		...
		else if (format == DBFTypes.DBASEIV_WITH_MEMO
				|| format == DBFTypes.DBASEIV || MDX_exist == 1)
			if (memoPresent)
				version = DBFTypes.DBASEIV_WITH_MEMO;
			else
	...
	public void getXML(PrintWriter pw) throws IOException, xBaseJException {
		...
		int i;
		pw.println("<dbf name=\"" + Util.normalize(getName())
	...
	protected void openDBF(String DBFname) throws IOException, xBaseJException {
		...
		String fptname = new String(dosname.substring(0, dosname.length() - 3)
				+ "fpt");
		File f = new File(fptname);
		if (f.exists() && !f.isDirectory()) {
			version = DBFTypes.FOXPRO_WITH_MEMO;
			System.out.println("Version corrected to "
					+ DBFTypes.FOXPRO_WITH_MEMO);
		}
		...
		if ((version != DBFTypes.DBASEIII)
				...
				&& (version != DBFTypes.DBASEIV_WITH_MEMO)
				&& (version != DBFTypes.FOXPRO_WITH_MEMO)) {
			String mismatch = Util.getxBaseJProperty("ignoreVersionMismatch")
					.toLowerCase();
			if (mismatch != null
		...
		if (fldcount != fldcount_calc) {
			System.out.println("Field count corrected from " + fldcount
					+ " to " + fldcount_calc);
		...
		if (MDX_exist == 1) {
			try {
				...
				String missing = Util.getxBaseJProperty("ignoreMissingMDX")
						.toLowerCase();
				if (missing != null
	...
	public void write(boolean lock) throws xBaseJException, IOException {
		...
		if (MDX_exist != 1
				&& (version == DBFTypes.DBASEIII || version == DBFTypes.DBASEIII_WITH_MEMO)) {
			buffer.position(0);
			channel.write(buffer);
			wb = ' ';
			for (i = 0; i < lrecl; i++)
